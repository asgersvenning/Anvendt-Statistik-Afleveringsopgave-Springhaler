...
```
We see from the __Normal Q-Q plot__ that the dataset is approximatly normal distrubuted. 

On the graph that shows residuals vs. fitted indicate the model have some fauls. This graph indicate that our model is better at predicting lower values then higher values. But there isn't at clear tendensy, and therefor the model is accepted.  
...
$$T(h,y)=\frac{1}{-0.5942}\frac{h}{y}-\frac{0.8025}{-0.5942}\cdot h - \frac{14.7140}{-0.5942}$$
...
test %>%   
  filter(between(Temperature,5,20),Osmolality<(0.8025)^-1) %>% 
  ...
  coord_cartesian(ylim=c(0,(0.8025)^-1), xlim=c(0,400), expand=F) +
  theme_minimal() +
